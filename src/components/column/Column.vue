...
const onDragChange = (evt: any) => {
  if (evt.added) {
    ...
    taskStore.moveTask(element.id, element.column_id, props.column.id, newIndex)
  } else if (evt.moved) {
...
<template>
  <div class="w-80 flex-none flex flex-col max-h-full bg-[#f1f5f9] rounded-2xl border border-slate-200 shadow-sm group/column">
    <!-- Column Header -->
    <div class="p-4 flex items-center justify-between column-handle cursor-grab active:cursor-grabbing">
      <div class="flex items-center gap-2 flex-1 min-w-0">
        <GripVertical class="w-4 h-4 text-slate-400 opacity-0 group-hover/column:opacity-100 transition-opacity" />
        <EditableTitle
      ...
      <Button variant="ghost" size="icon" class="h-8 w-8 text-slate-400 hover:text-destructive" @click="handleDeleteColumn">
    ...
    <div class="p-3 mt-auto">
      <Button
        variant="ghost"
        class="w-full justify-start text-slate-500 hover:text-primary hover:bg-primary/5 rounded-xl font-medium"
        @click="handleAddTask"
      >
