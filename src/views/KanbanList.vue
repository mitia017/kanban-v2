...
const handleDelete = async (id: number, e: Event) => {
  ...
  if (confirm('Are you sure you want to delete this board?')) {
    await kanbanStore.deleteKanban(id)
  }
}
  ...
  <div class="container py-10 max-w-6xl">
    <div class="flex items-center justify-between mb-10">
      <div>
        ...
        <p class="text-slate-500 mt-2">Manage your projects and stay organized.</p>
      </div>
      <Button @click="isCreateOpen = true" class="rounded-xl shadow-lg hover:translate-y-[-2px] transition-transform">
        <Plus class="w-5 h-5 mr-2" />
        New Board
      </Button>
    </div>

    <div v-if="kanbanStore.loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <Skeleton v-for="i in 3" :key="i" class="h-48 rounded-2xl" />
    </div>

    <div v-else-if="kanbanStore.kanbans.length === 0" class="text-center py-20 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
      ...
      <p class="text-slate-500 mt-2 mb-6">Create your first kanban board to get started.</p>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        ...
        <div class="flex items-center text-primary font-medium text-sm mt-4">
          Open Board
          <ExternalLink class="w-4 h-4 ml-2 opacity-0 group-hover:opacity-100 transition-all translate-x-[-10px] group-hover:translate-x-0" />
        </div>
      </Card>
    </div>

    <Dialog v-model:open="isCreateOpen" title="Create New Board" description="Start a new project board.">
      <div class="space-y-4 py-4">
        ...
        <div class="space-y-2">
          ...
          <Input v-model="newKanbanDescription" placeholder="What is this board about?" />
        </div>
      </div>
      <div class="flex justify-end gap-3 mt-4">
        <Button variant="outline" @click="isCreateOpen = false">Cancel</Button>
        <Button @click="handleCreate" :disabled="!newKanbanTitle.trim()">Create Board</Button>
      </div>
    </Dialog>
  </div>
</template>
